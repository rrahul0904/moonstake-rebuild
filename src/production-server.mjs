...
import {
  ...
  updateModeration, listAdminClaims, getAdminClaim, recordClaimRefund, consumeRateLimit, assertSupabaseConfig
} from './supabase.mjs';
...
async function checkRateLimit(req,key,limit=10,windowSeconds=600){const keyHash=crypto.createHash('sha256').update(`${clientIp(req)}|${req.headers['user-agent']||''}`).digest('hex');return Boolean(await consumeRateLimit({bucket:key,keyHash,limit,windowSeconds}))}
