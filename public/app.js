...
async function showPanel(tab){
  ...
  if(tab==='board'){
    $('#panel-kicker').textContent='THE BOARD';
    $('#panel-title').textContent='Brands & Moon Index';
    try{
      const [data,indexData,activityData]=await Promise.all([
        api('/api/board'),
        api('/api/index-history?limit=120').catch(()=>({points:[]})),
        api('/api/activity?limit=12').catch(()=>({activity:[]})),
      ]);
      const boardRows=data.board.length
        ? data.board.map((row,i)=>`<div class="board-row" data-claim="${row.id}"><span class="rank">${String(i+1).padStart(2,'0')}</span><div class="row-copy"><strong>${escapeHtml(row.brand)}</strong><small>Atlas label · ${escapeHtml(row.tagline||'No tagline yet')}</small></div><div class="row-metrics">${row.views} <small>views · ${row.clicks} clicks</small></div></div>`).join('')
        : '<div class="panel-empty">No brands are on the board yet.</div>';
      ...
      $('#panel-content').innerHTML=indexSparkline(indexData.points||[])
        +`<div class="panel-section">${boardRows}</div>`
        +`<div class="panel-section"><strong>Recent market activity</strong>${activity||'<div class="panel-empty">No paid market activity yet.</div>'}</div>`;
      $$('[data-claim]').forEach(el=>el.onclick=()=>{
        const claim=state.claims.find(x=>x.id===el.dataset.claim);
        if(claim){
          const pos=displayCoords(claim.sectors[0]);
          if(pos){focusSector(pos.x,pos.y);positionFlagCard(claim);}
        }
      });
    }catch(e){
  ...
  if(tab==='explore'){
    $('#panel-kicker').textContent='EXPLORE';
    $('#panel-title').textContent='Landmarks & flags';
    try{
      const data=await api('/api/explore');
      $('#panel-content').innerHTML=`<div class="panel-section">${data.landmarks.map(l=>`<div class="land-row" data-landmark="${l.id}"><span class="rank">◎</span><div class="row-copy"><strong>${escapeHtml(l.name)}</strong><small>Reference landmark · ${escapeHtml(l.subtitle)}</small></div><div class="row-metrics">${l.lat.toFixed(1)}°<small>${l.lon.toFixed(1)}°</small></div></div>`).join('')}</div>`;
      $$('[data-landmark]').forEach(el=>el.onclick=()=>{
        const landmark=state.landmarks.find(x=>x.id===el.dataset.landmark);
        if(landmark){
          focusSector(landmark.lotX??landmark.x,landmark.lotY??landmark.y,3.2);
  ...
  if(tab==='worlds'){
    ...
    try{
      ...
      const rows=data.bodies.map(body=>{
        ...
        const mode=body.inventoryMode==='surface-lots'
          ? 'surface registry'
          : body.inventoryMode==='reference-only'
            ? 'reference only'
            : 'observation registry';
        const lots=body.totalLots
          ? Number(body.totalLots).toLocaleString()+' positions'
          : mode;
  ...
  if(tab==='offers'){
    ...
    try{
      ...
      const payoutReady=Boolean(connect.enabled&&payout.resale_payout_ready);
      const payoutText=payoutReady?'Seller payouts ready':`Seller payouts: ${payout.onboarding_status||'not started'}`;
      const payoutAction=payoutReady
      ...
      const received=(data.received||[]).map(o=>{
        const action=o.status==='pending'
          ? `<button class="ghost-btn" data-accept-offer="${o.id}" ${payoutReady?'':'disabled'}>Accept</button>`
          : '';
      ...
      $('#panel-content').innerHTML=`<div class="panel-empty">${escapeHtml(payoutText)}${payoutReady?'':' · payout onboarding must be enabled before you can accept cash resale offers.'}<div class="panel-cta">${payoutAction}</div></div><div class="panel-section"><strong>Received</strong>${received||'<div class="panel-empty">No received offers.</div>'}</div><div class="panel-section"><strong>Sent</strong>${sent||'<div class="panel-empty">No sent offers.</div>'}</div><div class="panel-section"><strong>Watchlist</strong>${watched||'<div class="panel-empty">No watched positions.</div>'}</div>`;
