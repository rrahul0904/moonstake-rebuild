...
let claims=[],users=[],market={summary:null,offers:[],transactions:[]},view='claims';
...
function render(){
 ...
 $('#claims').hidden=view!=='claims';$('#users').hidden=view!=='users';$('#market').hidden=view!=='market';
 if(view==='claims'){
...
async function load(){try{$('#status').textContent='Loading operator data…';const [c,u,m]=await Promise.all([api('/api/admin/claims'),api('/api/admin/users'),api('/api/admin/marketplace')]);claims=c.claims||[];users=u.users||[];market=m||{summary:null,offers:[],transactions:[]};$('#status').textContent=`Administrator access confirmed · ${users.length} users · ${claims.length} claims · ${(market.transactions||[]).length} market transactions`;render()}catch(err){$('#status').innerHTML=`<span class="notice">${esc(err.message)}</span> — sign in on the main registry with an email included in ADMIN_EMAILS.`;$('#claims').innerHTML='<div class="empty">Operator data is unavailable.</div>';$('#users').innerHTML=''}}
